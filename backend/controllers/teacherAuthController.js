...
const updateProfile = async (req, res) => {
  try {
    ...
    if (email) {
        const emailExists = await Teacher.findOne({ where: { email: email.trim().toLowerCase() } });
        if (emailExists && emailExists.id !== teacher.id) {
            return res.status(400).json({ success: false, message: "Email already in use" });
        }
        teacher.email = email.trim().toLowerCase();
    }
    if (phone !== undefined) teacher.phone = phone ? phone.trim() : null;
...
const changePassword = async (req, res) => {
  try {
    ...
    const teacher = await Teacher.scope('withPassword').findByPk(req.user.id);
    if (!teacher) {
      return res.status(404).json({ success: false, message: "Teacher not found" });
    }
