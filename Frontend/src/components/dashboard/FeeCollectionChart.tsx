...
import { mockFeeCollection } from '../../data/mockData';
      ...
      <div>
        ...
        <div className="h-48 w-full relative flex items-center justify-center my-2">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie
                ...
                dataKey="value"
              >
                {mockFeeCollection.map((entry, index) => (
                  <Cell key={`cell-${index}`} fill={entry.color} stroke="none" />
                ))}
              </Pie>
              <Tooltip
                formatter={(value: any) => [`₹${value} Lakhs`, 'Amount']}
                contentStyle={{
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            ...
            <span className="text-lg font-extrabold text-slate-900">₹22.7L</span>
          </div>
        </div>
      </div>
