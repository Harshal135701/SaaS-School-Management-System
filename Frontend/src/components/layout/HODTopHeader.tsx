...
interface HODTopHeaderProps {
  onMenuClick: () => void;
  user?: any;
}

export const HODTopHeader: React.FC<HODTopHeaderProps> = ({ 
  onMenuClick,
  user
}) => {
  return (
    <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 sticky top-0 z-20 shadow-sm">
      <div className="flex items-center gap-4">
        ...
        <div className="hidden md:flex items-center gap-2 text-slate-500 bg-slate-100 px-3 py-1.5 rounded-lg border border-slate-200 focus-within:bg-white focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-100 transition-all w-64">
          ...
          <input 
            type="text" 
            placeholder="Search department, teachers..." 
      ...
      <div className="flex items-center gap-3 md:gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="hidden md:block text-right">
            <p className="text-sm font-bold text-slate-900 leading-none">{user?.name || 'HOD'}</p>
            <p className="text-[11px] text-slate-500 font-medium mt-0.5">{user?.email || 'hod@school.com'}</p>
          </div>
          <Avatar name={user?.name || 'HOD'} size="md" status="online" className="shadow-sm border border-slate-200" />
        </div>
      </div>
    </header>
  );
