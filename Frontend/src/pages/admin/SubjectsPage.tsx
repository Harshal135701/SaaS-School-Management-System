...
export const SubjectsPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingSubject) {
        await api.put(
          `/franchise/subjects/${editingSubject.id}`,
          payload
        );
      } else {
        await api.post(
  ...
  const handleDelete = async (sub: SubjectItem) => {
    if (
      !window.confirm(
        `Are you sure you want to deactivate subject "${sub.name}"?`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      await api.delete(
        `/franchise/subjects/${sub.id}`
      );

      await fetchSubjects();
    } catch (err: any) {
      console.error(
        'Error deactivating subject:',
        err
      );
  ...
  const handleReactivate = async (
    ...
    if (
      !window.confirm(
        `Are you sure you want to reactivate subject "${sub.name}"?`
      )
    ) {
      return;
    }
        ...
        <div className="py-16 text-center rounded-2xl bg-white border border-dashed border-slate-200 p-6">
          ...
          <p className="text-xs text-slate-400 mt-1">
            {search
              ...
              : 'Click "Add Subject" to define your curriculum.'}
          </p>
        </div>
      ) : (
